...
public abstract class MixinTileEntity implements ICapabilitySerializable<NBTTagCompound> {
    ...
    private boolean preventTeCrashes(CrashReportCategory category, BlockPos pos, @Nullable Block block, int metadata) {
        if (block == null) {
            PaperFixes.LOGGER.error("A tile entity with a null block was found at {}. "
                    + "It will be excluded from the crash report to prevent additional issues.", pos.toString());
            return false;
        }
        return true;
    }
